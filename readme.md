...
- Auto-detects Debian live versus Fedora Workstation Live environments
...
- Fedora target release is auto-detected from the Fedora Workstation Live environment
...
- **Host media**: Fedora Workstation Live is required as the host because the installer needs a full Fedora live userspace with `dnf` available.
- **Target profile**: Fedora installs are built minimally with `dnf --installroot`; the host live image does not determine the installed profile, no desktop environment is copied into the target and removed later, and the installer attempts to apply Fedora Server release identity packages when they are available.
- **Guide alignment**: The Fedora path uses Fedora Workstation Live as host media, like the official ZFSBootMenu guide, but diverges by building a minimal headless target instead of rsyncing the Workstation root.
...
- [InstallZFSBootMenuFedora.txt](InstallZFSBootMenuFedora.txt) opens a terminal in Fedora Workstation Live and bootstraps the installer from `feature/fedora-support`.
...
- **Fedora host media**: If the script says it cannot find `dnf`, `dnf5`, or `microdnf`, you are not in a full Fedora live environment. Boot Fedora Workstation Live and rerun it there.
